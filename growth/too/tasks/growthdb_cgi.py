...
def get_source_autoannotations(sourceid):
    ...
    autoannotations_string = '; '.join(f"{auto['username']}, {auto['type']}, {auto['comment']}" for auto in autoannotations)
...
def get_candidates_growth_marshal(program_name):
    ...
    for source in r.json():
        yield dict(source, annotations=get_source_autoannotations(source["id"]))
