...
def objects(dateobs):
    ...
    if request.method == 'POST':
        if 'btngcn' in request.form:
            x = PrettyTable()
            x.field_names = ["Name", "IAU Name", "RA", "Dec", "filter", "mag",
                             "MJD", "Notes"]
            ...
            for source_dict in sources_growth_marshal_contour:
                if source_dict["name"] in checked:
                    ...
                    x.add_row([source_dict["name"],
                               ...
                               source_dict["ra"].to_string(u.hour,precision=2, sep=':', pad=True),
                               source_dict["dec"].to_string(precision=2, sep=':', pad=True, alwayssign=True),
